...
"""
        ...
        # Scoped by session_id so one user/session can never end up being
        # served a resume generated by a different session — the output
        # folder used to be shared globally, which let stale or unrelated
        # sessions' files leak into someone else's chat.
        docx_dir = os.path.join(os.getcwd(), "output", "resumes", str(session_id))
