...
def _count_tokens(text: str) -> int:
    """Estimate token count from text length using a simple character-based heuristic.

    Args:
        text: The text to estimate.

    Returns:
        An approximate token count, with a minimum value of 1.
    """
    if not text:
        return 1
...
def log_request(
    *,
    ...
    """Log a single LLM request to disk and to MLflow for observability.

    Args:
        model: Name of the model used for the request.
        prompt: The prompt or user message that was sent.
        completion: The model's response.
        latency_ms: Request latency in milliseconds.
        guardrail_fired: Whether a guardrail blocked or modified the request.
        extra: Optional metadata to include in the log record.
    """
    try:
        ...
        with mlflow.start_run(run_name=f"faq_{int(time.time())}"):
            ...
            # Parameters — tracking the text snippets directly in the UI columns.
            mlflow.log_params({
                "model": model,
